...
begin
  ...
  begin
    try
      ...
      EResNotFound do
      begin
        MessageBox(0, PChar('File not found in resource - ' + Filename), PChar('PNG Texture'), MB_OK);
        Exit;
      end
      else
      begin
        MessageBox(0, PChar('Couldn''t load PNG Resource - "'+ Filename +'"'), PChar('BMP Unit'), MB_OK);
        Exit;
      end;
    end;
  end
  ...
  BMP.canvas.draw(0,0,PNG);        // Copy the JPEG onto the Bitmap
