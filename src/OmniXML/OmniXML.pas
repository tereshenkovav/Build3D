...
uses
  Classes, SysUtils, {$IFDEF OmniXML_D2009_UP}Types,{$ENDIF}
...
type
  ...
  IUnicodeWriteStream = interface
  ['{05CD3CDD-A218-4663-B6F1-376374A8BD06}']
  //private
  ...
  IUnicodeReadStream = interface
    ['{37C525D4-443B-45D6-A13B-9551790AA495}']
    // private
    function GetEncoding: TEncoding;
    ...
    property Encoding: TEncoding read GetEncoding write SetEncoding;
    procedure UndoRead;
    function ReadChar(var aReadChar: XmlChar): Boolean;
...
type
  ...
  public
    function ReadChar(var aReadChar: XmlChar): Boolean;
    procedure UndoRead;
    function GetNextString(var aReadString: XmlString; const aLength: Integer): Boolean;
  ...
  TXMLWriteStream = class(TInterfacedObject, IUnicodeWriteStream)
  private
  ...
  IXMLNode = interface
    ...
    // private
    function GetAttributes: IXMLAttrList;
    ...
    function AddChild(const aTagName: XmlString): IXMLElement;//ONDREJ
    function AddText(const aText: XmlString): IXMLText;//ONDREJ
    function InsertChild(const aTagName: XmlString; aBeforeChild: IXMLNode): IXMLElement;//ONDREJ
    procedure RemoveChildren(const aTagName: XmlString);//ONDREJ
    ...
    property Attributes: IXMLAttrList read GetAttributes;
    ...
    procedure WriteToStream(const OutputStream: IUnicodeWriteStream);
    procedure SelectSingleNode(const Pattern: XmlString; var Result: IXMLNode); overload;
    function SelectSingleNode(const Pattern: XmlString): IXMLNode; overload;
    function FindNode(const Pattern: XmlString): IXMLElement;//ONDREJ
    function SelectSingleElement(const Pattern: XmlString): IXMLElement;//ONDREJ
    function SelectSingleElementNil(const Pattern: XmlString): IXMLElement;//ONDREJ
    function SelectSingleElementCreate(const aTagName: XmlString): IXMLElement;//ONDREJ
    procedure SelectNodes(const Pattern: XmlString; var Result: IXMLNodeList); overload;
    function SelectNodes(const Pattern: XmlString): IXMLNodeList; overload;

    function GetXML: XmlString;//ondrej
    function GetXMLFormatted(const OutputFormat: TOutputFormat): XmlString;//ondrej
    property XML: XmlString read GetXML;
    {$IFNDEF OmniXML_NEXTGEN}
    function GetXML_UTF8: XmlRawByteString;//ondrej
    function GetXMLFormatted_UTF8(const OutputFormat: TOutputFormat): XmlRawByteString;//ondrej
    property XML_UTF8: XmlRawByteString read GetXML_UTF8;
    {$ENDIF}
    {$IFDEF OmniXML_D2009_UP}
    function GetXML_UTF8Bytes: TBytes;//ondrej
    function GetXMLFormatted_UTF8Bytes(const OutputFormat: TOutputFormat): TBytes;//ondrej
    property XML_UTF8Bytes: TBytes read GetXML_UTF8Bytes;
    {$ENDIF}
    function GetXML_Unicode: XmlWideString;//ondrej
    function GetXMLFormatted_Unicode(const OutputFormat: TOutputFormat): XmlWideString;//ondrej
    property XML_Unicode: XmlWideString read GetXML_Unicode;

    function GetAttribute(const Name: XmlString): XmlString;//ondrej
    function GetAttributeDef(const Name, aDefValue: XmlString): XmlString;//ondrej
    function SetAttribute(const Name, Value: XmlString): IXmlNode;//ondrej; returns self
    function RemoveAttribute(const Name: XmlString): IXmlNode;//ondrej; returns self
    function GetAttributeNode(const Name: XmlString): IXMLAttr;//ondrej
    function SetAttributeNode(const NewAttr: IXMLAttr): IXMLAttr;//ondrej
    function RemoveAttributeNode(const OldAttr: IXMLAttr): IXMLAttr;//ondrej
    function HasAttribute(const Name: XmlString): Boolean;//ondrej

    function IsTextElement: Boolean;//ONDREJ
    function IsNodeElement: Boolean;//ONDREJ
  end;
  ...
  IXMLCustomList = interface
    ...
    function Get(const Index: Integer): IXMLNode;//ondrej - TXMLDocument compatibility reasons
    // public
    property Item[const Index: Integer]: IXMLNode read GetItem;//ondrej
    property Length: Integer read GetLength;
    property Count: Integer read GetLength;//ondrej
    ...
    function FindNode(const aNodeName: XmlString): IXMLNode;//rs
  end;
  ...
  IXMLNodeList = interface(IXMLCustomList)
    ...
    property Item[const Index: Integer]: IXMLNode read GetItem; default;//ondrej

  end;

  TXMLAttrListEnumerator = class(TObject)
  private
    fIndex: Integer;
    fList: IXMLAttrList;
  public
    constructor Create(aList: IXMLAttrList);
    function GetCurrent: IXMLAttr;
    function MoveNext: Boolean;
  public
    property Current: IXMLAttr read GetCurrent;
  end;

  IXMLAttrList = interface(IXMLCustomList)
    ['{87964B1D-F6CC-46D2-A602-67E198C8BFF5}']
    // public
    function GetNamedItem(const Name: XmlString): IXMLAttr;
    function SetNamedItem(const Arg: IXMLAttr): IXMLAttr;
    function RemoveNamedItem(const Name: XmlString): IXMLAttr;

    function GetValue(const Name: XmlString): XmlString;//ondrej
    procedure SetValue(const Name, Value: XmlString);//ondrej

    function GetEnumerator: TXMLAttrListEnumerator;

    property Values[const Name: XmlString]: XmlString read GetValue write SetValue; default;//ondrej
  end;
...
(*
  IXMLDocumentType = interface(IXMLNode)
    ...
    // private
    function GetEntities: IXMLAttrList;
    function GetName: XmlString;
    function GetNotations: IXMLAttrList;
    // public
    property Name: XmlString read GetName;
    property Entities: IXMLAttrList read GetEntities;
    property Notations: IXMLAttrList read GetNotations;
  end;
*)
  ...
  //wsTrim = trim all
  //wsPreserveAll = preserve all white spaces (also whitespace outside tags)
  //wsPreserveInTextOnly = preserve whitespace if some text is inside (delete empty nodes and empty space for indentation ect.)
  //wsAutoTag = use xml:preserve tag for decision
...
//    procedure SetDocType(const Value: IXMLDocumentType);
    ...
    function LoadFromFile(const FileName: string): Boolean;//ONDREJ
    function LoadFromStream(const Stream: TStream): Boolean; overload;
    function LoadFromStream(const Stream: TStream; const aForceEncoding: Word): Boolean; overload;
    procedure Save(const FileName: string; const OutputFormat: TOutputFormat = ofNone);
    procedure SaveToFile(const FileName: string; const OutputFormat: TOutputFormat = ofNone);//ONDREJ
    ...
    {$IFNDEF OmniXML_NEXTGEN}
    function LoadXML_UTF8(const XML: XmlRawByteString): Boolean;
    {$ENDIF}
    {$IFDEF OmniXML_D2009_UP}
    function LoadXML_UTF8Bytes(const XML: TBytes): Boolean;
    {$ENDIF}
    function GetParseError: IXMLParseError;
    property ParseError: IXMLParseError read GetParseError;

    //ONDREJ - to get back compatibility for older versions of OmniXML
...
type
  ...
  protected
    ...
    function GetParentNode: IXMLNode;
    function GetPreviousSibling: IXMLNode;
    procedure SetNodeValue({%H-}const Value: XmlString); virtual;
    procedure InternalWrite(const Stream: TStream; Text: XmlString); virtual;
    procedure InternalWriteToStream(const OutputStream: IUnicodeWriteStream); virtual;
    procedure ReadFromStream({%H-}const Parent: TXMLNode; {%H-}const InputStream: IUnicodeReadStream); virtual;
    procedure SetParentNode(const Parent: IXMLNode); virtual;//ONDREJ
    function GetText: XmlString; virtual;
    procedure SetText(const Value: XmlString); virtual;
    procedure SelectNodes(const Pattern: XmlString; var Result: IXMLNodeList); overload; virtual;
    procedure SelectSingleNode(const Pattern: XmlString; var Result: IXMLNode); overload; virtual;
  public
    ...
    procedure WriteToStream(const OutputStream: IUnicodeWriteStream);
    function SelectNodes(const Pattern: XmlString): IXMLNodeList; overload; virtual;
    function SelectSingleNode(const Pattern: XmlString): IXMLNode; overload; virtual;
    function FindNode(const Pattern: XmlString): IXMLElement;//ONDREJ
    function SelectSingleElement(const Pattern: XmlString): IXMLElement;//ONDREJ
    function SelectSingleElementNil(const Pattern: XmlString): IXMLElement;//ONDREJ
    function SelectSingleElementCreate(const aTagName: XmlString): IXMLElement;//ONDREJ

    function GetXML: XmlString; overload;
    function GetXMLFormatted(const OutputFormat: TOutputFormat): XmlString; overload;
    property XML: XmlString read GetXML;
    {$IFNDEF OmniXML_NEXTGEN}
    function GetXML_UTF8: XmlRawByteString;//ondrej
    function GetXMLFormatted_UTF8(const OutputFormat: TOutputFormat): XmlRawByteString;//ondrej
    ...
    function GetAttribute(const Name: XmlString): XmlString;//ondrej
    function GetAttributeDef(const Name, aDefValue: XmlString): XmlString;//ondrej
    function SetAttribute(const Name, Value: XmlString): IXMLNode;//ondrej
    function RemoveAttribute(const Name: XmlString): IXMLNode;//ondrej
    function GetAttributeNode(const Name: XmlString): IXMLAttr;//ondrej
    function SetAttributeNode({%H-}const NewAttr: IXMLAttr): IXMLAttr;//ondrej
    function RemoveAttributeNode(const OldAttr: IXMLAttr): IXMLAttr;//ondrej
    function HasAttribute(const Name: XmlString): Boolean;//ondrej
...
*)
  ...
  protected
    ...
    procedure ReadFromStream(const {%H-}Parent: TXMLNode; const InputStream: IUnicodeReadStream); override;
  public
  ...
  private
    {$IFDEF OmniXML_InterfaceListOptimalization}
    FList: TList;
    {$ELSE}
  ...
  public
    function Get(const Index: Integer): IXMLNode;
  public
    constructor Create;
    destructor Destroy; override;
  ...
  TXMLAttrList = class(TXMLCustomList, IXMLAttrList)
  private
    Parent: Pointer;//IXMLNode;//Ondrej -> BS: memory leak (nodes would not be destroyed - use as pointers - reference count is not incremented)
  public
    function GetNamedItem(const Name: XmlString): IXMLAttr;
    function SetNamedItem(const Arg: IXMLAttr): IXMLAttr;
    function RemoveNamedItem(const Name: XmlString): IXMLAttr;
    ...
    constructor Create(const aParent: IXMLNode);//IXMLNode
    destructor Destroy; override;
    //property Values[const Name: XmlString]: XmlString read GetValue write SetValue; default;//ondrej
  end;
  ...
  protected
    function GetNodeName: XmlString; override;
    function GetTagName: XmlString;
    procedure InternalWriteToStream(const OutputStream: IUnicodeWriteStream); override;
    procedure ReadFromStream({%H-}const Parent: TXMLNode; const InputStream: IUnicodeReadStream); override;
    procedure SetTagName(const TagName: XmlString);

    //ONDREJ
    procedure SetParentNode(const Parent: IXMLNode); override;//ONDREJ
    function GetNodeValue: XmlString; override;
    procedure SetText(const Value: XmlString); override;
    function GetPreserveWhiteSpace: Boolean;
    procedure SetPreserveWhiteSpace(const Value: Boolean);
  public
    property TagName: XmlString read GetTagName;
...
//    function CloneNode(const Deep: Boolean): IXMLNode; override;
  ...
  protected
    ...
    procedure InternalWriteToStream(const OutputStream: IUnicodeWriteStream); override;
    procedure ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream); override;
  public
    property Target: XmlString read GetTarget;
    property Data: XmlString read GetData;
  ...
  protected
    ...
    procedure SetNodeValue(const Value: XmlString); override;
    procedure InternalWriteToStream(const OutputStream: IUnicodeWriteStream); override;
    procedure ReadFromStream(const {%H-}Parent: TXMLNode; const {%H-}InputStream: IUnicodeReadStream); override;
  ...
  public
    ...
    function SubstringData(const {%H-}Offset, {%H-}Count: Integer): XmlString;
    procedure AppendData({%H-}const Arg: XmlString);
    procedure InsertData({%H-}const Offset: Integer; const {%H-}Arg: XmlString);
    procedure DeleteData({%H-}const Offset, {%H-}Count: Integer);
    procedure ReplaceData({%H-}const Offset, {%H-}Count: Integer; const {%H-}Arg: XmlString);
  end;
  ...
  public
    ...
    function SplitText(const {%H-}Offset: Integer): IXMLText;
  end;
...
//    procedure SetDocType(const Value: IXMLDocumentType);
    ...
    function _LoadFromStream(const Stream: TStream; const aForceXMLEncoding: Word = 0): Boolean;
  protected
    ...
    function InternalCreateEntityReference(const {%H-}Name: XmlString): TXMLEntityReference;
  ...
  public
    ...
    property DocType: IXMLDocumentType read GetDocType;
    property DocumentElement: IXMLElement read GetDocumentElement write SetDocumentElement;
    constructor Create({%H-}AParent: Pointer = nil); virtual; //rs
    destructor Destroy; override;
    function CreateAttribute(const Name: XmlString): IXMLAttr;
    function CreateCDATASection(const Data: XmlString): IXMLCDATASection;
    function CreateComment(const Data: XmlString): IXMLComment;
    ...
    procedure SaveToFile(const FileName: string; const OutputFormat: TOutputFormat = ofNone);
    procedure SaveToStream(const OutputStream: TStream; const OutputFormat: TOutputFormat = ofNone);
    function LoadXML(const XML: XmlString): Boolean; virtual;
    {$IFNDEF OmniXML_NEXTGEN}
    function LoadXML_UTF8(const XML: XmlRawByteString): Boolean; virtual;
    {$ENDIF}
    {$IFDEF OmniXML_D2009_UP}
    function LoadXML_UTF8Bytes(const XML: TBytes): Boolean; virtual;
    {$ENDIF}
    property ParseError: IXMLParseError read GetParseError;
  ...
  TVirtualMemoryStream = class(TCustomMemoryStream)//MUST BE TCustomMemoryStream -> SO THAT THE MEMORY POINTER WOULD NOT GET DESTROYED IN .Free!!!
  public
    procedure SetPointer(Ptr: Pointer; const Size: Longint); reintroduce;//public
    function Write(const {%H-}Buffer; {%H-}Count: Longint): Longint; override;
  end;

// helper functions
function CreateXMLDoc: IXMLDocument; overload;
function CreateXMLDoc(const aRootNodeName: String): IXMLDocument; overload;
function CreateXMLDoc(const aRootNodeName, aProcessingInstruction: String): IXMLDocument; overload;
function CreateXMLDoc(const aRootNodeName: String; const aAddUTF8ProcessingInstruction: Boolean): IXMLDocument; overload;
// Unicode functions
...
function StringNeedsPreserveTag(const str: XmlString): Boolean;
...
uses
  ...
  {$IFDEF FPC}, lazutf8classes{$ENDIF};
...
type
  ...
  {$IFDEF FPC}
  TXmlFileStream = TFileStreamUTF8;
  {$ELSE}
  TXmlFileStream = TFileStream;
  {$ENDIF}
...
function CreateXMLDoc(const aRootNodeName: String): IXMLDocument; overload;
begin
  Result := CreateXMLDoc(aRootNodeName, '');
end;

function CreateXMLDoc(const aRootNodeName: String; const aAddUTF8ProcessingInstruction: Boolean): IXMLDocument;
begin
  if aAddUTF8ProcessingInstruction then
    Result := CreateXMLDoc(aRootNodeName, 'version="1.0" encoding="UTF-8" standalone="yes"')
  else
    Result := CreateXMLDoc(aRootNodeName, '');
end;
...
begin
  Result := CreateXMLDoc;
  if aProcessingInstruction <> '' then
    Result.AppendChild(Result.CreateProcessingInstruction('xml', aProcessingInstruction));

  Result.DocumentElement := Result.CreateElement(aRootNodeName);
end;

function FindCreateEncoding(const PI: IXMLProcessingInstruction;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Result := GetCreateCodePage(Encoding, aEncoding);
        end;
      end;
    end;
  end;
...
begin
  ...
  Result := CharIs_WhiteSpace(str[1]) or CharIs_WhiteSpace(str[Length(str)]);
  if Result then
    Exit;
  ...
  while I < xLength do begin//we can omit first and last characters (already checked)!
    ...
    xThisCharWhiteSpace := CharIs_WhiteSpace(str[I]);
...
function Reference2Char(const InputStream: IUnicodeReadStream): XmlChar;
type
...
begin
  ...
  while InputStream.ReadChar({%H-}ReadChar) do
  begin
    case PState of
      psReference:
...
function StringIs_WhiteSpace(const Value: XmlString): Boolean;
var I: Integer;
begin
  for I := 1 to Length(Value) do
  if not CharIs_WhiteSpace(Value[I]) then begin
    Result := False;
    exit;
  end;
  Result := True;
...
function TXMLCustomList.FindNode(const aNodeName: XmlString): IXMLNode;
var
  I: Integer;
begin
  //rs
  for I := 0 to GetLength-1 do
  if (GetItem(I).NodeName = aNodeName) then begin
    Result := GetItem(I);
    Exit;
  end;
...
function TXMLCustomList.Get(const Index: Integer): IXMLNode;
begin
...
begin
  if Index < FList.Count then begin // BS
    {$IFDEF OmniXML_InterfaceListOptimalization}
...
constructor TXMLAttrList.Create(const aParent: IXMLNode);//IXMLNode);
begin
  inherited Create;
...
destructor TXMLAttrList.Destroy;
begin
  Parent := nil;
...
function TXMLAttrList.GetEnumerator: TXMLAttrListEnumerator;
begin
  Result := TXMLAttrListEnumerator.Create(Self);
end;

function TXMLAttrList.GetNamedItem(const Name: XmlString): IXMLAttr;
var
  i: Integer;
begin
  i := 0;
  ...
  begin
    if (Item[i].NodeName = Name) and Supports(Item[i], IXMLAttr) then
      Result := Item[i] as IXMLAttr
    else
      Inc(i);
  end;
end;

function TXMLAttrList.GetValue(const Name: XmlString): XmlString;
var xA: IXMLAttr;
begin
  xA := GetNamedItem(Name);
  if Assigned(xA) then
    Result := xA.Value
  else
    Result := '';
...
function TXMLAttrList.RemoveNamedItem(const Name: XmlString): IXMLAttr;
begin
...
function TXMLAttrList.SetNamedItem(const Arg: IXMLAttr): IXMLAttr;
var
  Index: Integer;
begin
...
procedure TXMLAttrList.SetValue(const Name, Value: XmlString);
var xA: IXMLAttr;
begin
  //(IInterface(Parent) as IXmlNode).SetAttribute(Name, Value);
  xA := GetNamedItem(Name);
  if not Assigned(xA) then begin
    xA := (IInterface(Parent) as IXmlNode).OwnerDocument.CreateAttribute(Name);
    Add(xA);
  end;
  xA.Value := Value;
end;
...
function TXMLNode.FindNode(const Pattern: XmlString): IXMLElement;
begin
  Result := SelectSingleElement(Pattern);
end;
...
function TXMLNode.HasAttribute(const Name: XmlString): Boolean;
begin
...
function TXMLNode.AddChild(const aTagName: XmlString): IXMLElement;
begin
  Result := Self.AppendChild(Self.OwnerDocument.CreateElement(aTagName)) as IXMLElement;
end;

function TXMLNode.AddText(const aText: XmlString): IXMLText;
begin
  if (ChildNodes.Count > 0) and Supports(ChildNodes[ChildNodes.Count-1], IXMLText, Result) then
    Result.Text := Result.Text + aText
  else
    Result := Self.AppendChild(Self.OwnerDocument.CreateTextNode(aText)) as IXMLText;
end;
...
function TXMLNode.GetAttributes: IXMLAttrList;
begin
  if FAttributes = nil then
    FAttributes := TXMLAttrList.Create(Self);
...
function TXMLNode.GetPreviousSibling: IXMLNode;
  function FindPreviousNode(const aSelf: IXMLNode): IXMLNode;
  var
    Childs: IXMLNodeList;
    Index: Integer;
  begin
    Childs := FParentNode.ChildNodes;
    Index := Childs.IndexOf(aSelf);
...
function TXMLNode.GetNextSibling: IXMLNode;
  function FindNextNode(const aSelf: IXMLNode): IXMLNode;
  var
    Childs: IXMLNodeList;
    Index: Integer;
  begin
...
function TXMLNode.InsertChild(const aTagName: XmlString;
  aBeforeChild: IXMLNode): IXMLElement;
begin
  Result := Self.InsertBefore(Self.OwnerDocument.CreateElement(aTagName), aBeforeChild) as IXMLElement;
end;
...
procedure TXMLNode.RemoveChildren(const aTagName: XmlString);
var
  I: Integer;
begin
  try
    for I := Self.ChildNodes.Count-1 downto 0 do
    if Self.ChildNodes[I].NodeName = aTagName then begin
      Self.ChildNodes.Delete(I);
      //cannot be break - delete all!
    end;
  except
    //nothing
  end;
...
procedure TXMLNode.ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream);
begin
  // do nothing
end;

procedure TXMLNode.InternalWriteToStream(const OutputStream: IUnicodeWriteStream);
var
  i: Integer;
begin
  if HasChildNodes then
...
function TXMLNode.IsNodeElement: Boolean;
begin
  Result := Supports(Self, IXMLElement);
end;

function TXMLNode.IsTextElement: Boolean;
begin
  Result := Supports(Self, IXMLText);
end;

procedure TXMLNode.WriteToStream(const OutputStream: IUnicodeWriteStream);
begin
...
function TXMLNode.GetXMLFormatted(const OutputFormat: TOutputFormat): XmlString;
begin
  {$IFDEF FPC}
  Result := GetXMLFormatted_UTF8(OutputFormat);
  {$ELSE}
  Result := GetXMLFormatted_Unicode(OutputFormat);
  {$ENDIF}
end;

function TXMLNode.GetXMLFormatted_Unicode(
  const OutputFormat: TOutputFormat): XmlWideString;
var
...
begin
  ...
  try
    US := TXMLWriteStream.Create(Stream);
    try
      US.Encoding := OEncoding_Unicode;
      US.WriteBOM := False;
    ...
    if Stream.Size > 0 then begin
      ...
      Stream.Read(PXmlWideChar(Result)^, Stream.Size);
    end;
  finally
    Stream.Free;
  end;
...
{$IFNDEF OmniXML_NEXTGEN}
function TXMLNode.GetXMLFormatted_UTF8(
  const OutputFormat: TOutputFormat): XmlRawByteString;
var
  Stream: TMemoryStream;
  US: TXMLWriteStream;
begin
  ...
  try
    US := TXMLWriteStream.Create(Stream);
    try
      US.Encoding := OEncoding_UTF8;
      US.OutputFormat := OutputFormat;
      US.WriteBOM := False;
      InternalWriteToStream(US);
    finally
      US.Free;//FREE -> MUST BE HERE TO WRITE TO MEMORY STREAM!!!
    end;

    SetLength(Result, Stream.Size);
    if Stream.Size > 0 then begin
      Stream.Seek(0, soFromBeginning);
      //OLD AnsiString format
...
function TXMLNode.GetXML_UTF8: XmlRawByteString;
begin
  Result := GetXMLFormatted_UTF8(ofNone);
end;

{$ENDIF}

{$IFDEF OmniXML_D2009_UP}
function TXMLNode.GetXMLFormatted_UTF8Bytes(
  const OutputFormat: TOutputFormat): TBytes;
var
  Stream: TMemoryStream;
  US: TXMLWriteStream;
begin
...
function TXMLNode.GetXML_UTF8Bytes: TBytes;
begin
  Result := GetXMLFormatted_UTF8Bytes(ofNone);
end;

{$ENDIF}

function TXMLNode.GetXML_Unicode: XmlWideString;
begin
  Result := GetXMLFormatted_Unicode(ofNone);
end;
...
procedure TXMLNode.SelectNodes(const Pattern: XmlString; var Result: IXMLNodeList);
begin
  Result := XPathSelect(Self, Pattern);
end;

function TXMLNode.SelectNodes(const Pattern: XmlString): IXMLNodeList;
begin
  SelectNodes(Pattern, {%H-}Result);
end;

procedure TXMLNode.SelectSingleNode(const Pattern: XmlString; var Result: IXMLNode);
var
...
function TXMLNode.SelectSingleElement(const Pattern: XmlString): IXMLElement;
var xRes: IXMLNode;
begin
  SelectSingleNode(Pattern, {%H-}xRes);
  if Assigned(xRes) and Supports(xRes, IXMLElement) then
    Result := xRes as IXMLElement
  else
    Result := nil;
end;

function TXMLNode.SelectSingleElementCreate(const aTagName: XmlString): IXMLElement;
var xRes: IXMLNode;
begin
  SelectSingleNode(aTagName, {%H-}xRes);
  if Assigned(xRes) and Supports(xRes, IXMLElement) then
    Result := xRes as IXMLElement
  else
...
var xNilDoc: IXMLDocument = nil;

function GetNilXMLElement: IXMLElement;
begin
  if not Assigned(xNilDoc) then begin
    xNilDoc := CreateXMLDoc;
    xNilDoc.DocumentElement := xNilDoc.CreateElement('nil');
  end;
  Result := xNilDoc.DocumentElement;
end;

function TXMLNode.SelectSingleElementNil(const Pattern: XmlString): IXMLElement;
var xRes: IXMLNode;
begin
  SelectSingleNode(Pattern, {%H-}xRes);
  if Assigned(xRes) and Supports(xRes, IXMLElement) then
    ...
    Result := GetNilXMLElement;
end;

function TXMLNode.SelectSingleNode(const Pattern: XmlString): IXMLNode;
begin
  SelectSingleNode(Pattern, {%H-}Result);
end;
...
function TXMLDocumentType.GetEntities: IXMLAttrList;
begin
...
function TXMLDocumentType.GetNotations: IXMLAttrList;
begin
...
function TXMLNode.GetAttribute(const Name: XmlString): XmlString;
var
  Attr: IXMLAttr;
begin
...
function TXMLNode.GetAttributeDef(const Name,
  aDefValue: XmlString): XmlString;
var xA: IXmlAttr;
begin
  xA := GetAttributeNode(Name);
  if Assigned(xA) then
    Result := xA.Value
  else
    Result := aDefValue;
...
function TXMLNode.GetAttributeNode(const Name: XmlString): IXMLAttr;
var
  i: Integer;
begin
  Result := nil;
...
function TXMLElement.GetNodeValue: XmlString;
begin
  if PreserveWhiteSpace then
    Result := inherited GetNodeValue
  else
    Result := ShrinkWhitespace(inherited GetNodeValue);
end;
...
procedure TXMLElement.SetText(const Value: XmlString);
begin
  inherited SetText(Value);

  if (FOwnerDocument.WhiteSpaceHandling = wsAutoTag) and
    StringNeedsPreserveTag(Value)
  then
...
function TXMLNode.RemoveAttribute(const Name: XmlString): IXMLNode;
var
  i: Integer;
begin
  if not HasAttributes then
...
function TXMLNode.RemoveAttributeNode(const OldAttr: IXMLAttr): IXMLAttr;
var
  i: Integer;
begin
  Result := nil;
  ...
  while i < Attributes.Length do
  begin
    if Attributes.Item[i] = OldAttr then
    begin
      Result := OldAttr;
      Attributes.Delete(i);
      Exit;
...
procedure TXMLElement.InternalWriteToStream(const OutputStream: IUnicodeWriteStream);
var
  i: Integer;
const
  PreserveWhiteSpaceToText: Array[false..true] of String = ('default', 'preserve');
begin
  // 2002-12-17 (mr): fixed indentation
  ...
  //ONDREJ BEGIN
  if
    (FOwnerDocument.WhiteSpaceHandling = wsAutoTag) and (
      (((Self as IXMLElement) = FOwnerDocument.DocumentElement) and (Self.PreserveWhiteSpace)) or
      (Assigned(FParentNode) and (Supports(FParentNode, IXMLElement)) and
        ((FParentNode as IXMLElement).PreserveWhiteSpace <> Self.PreserveWhiteSpace)))
  then
    OutputStream.WriteString(' xml:space="' + PreserveWhiteSpaceToText[Self.PreserveWhiteSpace] + '"');
  //ONDREJ END
...
function TXMLNode.SetAttribute(const Name, Value: XmlString): IXMLNode;
var
  Attr: IXMLAttr;
begin
...
function TXMLNode.SetAttributeNode(const NewAttr: IXMLAttr): IXMLAttr;
begin
  Result := nil;
  Assert(False, 'NYI - SetAttributeNode');
end;

procedure TXMLElement.SetParentNode(const Parent: IXMLNode);
begin
  inherited SetParentNode(Parent);

  //ONDREJ
  if Supports(Parent, IXMLElement) then
    PreserveWhiteSpace := (Parent as IXMLElement).PreserveWhiteSpace;
end;

procedure TXMLElement.SetPreserveWhiteSpace(const Value: Boolean);
begin
  FPreserveWhiteSpace := Value;
end;

procedure TXMLElement.ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream);
type
      ...
      psAttr:
        ...
        begin
          case ReadChar of
            ...
            // [41] Attribute
            // [5] Name
            if CharIs_Letter(ReadChar) or CharIs_Digit(ReadChar) or (ReadChar = '_') then//ONDREJ CharIs_Digit <- not XML standard, but sorry... for compatibility issues
            begin
              ...
              //BEGIN ONDREJ
              if (Self is TXMLElement) and (_nodeAttr.NodeName = 'xml:space') then begin
                if (_nodeAttr.NodeValue = 'preserve') then
                  Self.PreserveWhiteSpace := True
                else if (_nodeAttr.NodeValue = 'default') then
                  Self.PreserveWhiteSpace := False;
                Attributes.RemoveNamedItem('xml:space');
              end;
              //END ONDREJ
            end
            else
...
procedure TXMLProcessingInstruction.InternalWriteToStream(const OutputStream: IUnicodeWriteStream);
begin
  // 2002-12-17 (mr): fixed indentation
  OutputStream.WriteIndent;
...
procedure TXMLProcessingInstruction.ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream);
type
      ...
      psEndData:
        begin
          ...
          begin
            ...
            if (FOwnerDocument.FTextStreamForceEncoding = 0) and FindCreateEncoding(Self, {%H-}xEncoding) then begin//ONLY IF WE DON'T FORCE ANY ENCODING
              if InputStream.Encoding.EncodingName <> xEncoding.EncodingName then
              begin
                 InputStream.Encoding := xEncoding;
                 Parent.OwnerDocument.ChildNodes.Clear;
              end else begin
                if not TEncoding.IsStandardEncoding(xEncoding) then//Free created encoding -> it's not used!!!
                  xEncoding.Free;
              end;
            end;
            Exit;
...
begin
  ...
  //ONDREJ attributes always with PreserveWhiteSpace
  Result := NodeValue;
end;
...
procedure TXMLAttr.InternalWriteToStream(const OutputStream: IUnicodeWriteStream);
begin
  if NodeName = '' then
    raise EXMLException.CreateParseError(SYNTAX_ERR, MSG_E_BADDECLNAME, []);
  OutputStream.WriteString(' ' + NodeName + '="' + EncodeText(NodeValue) + '"');
end;

procedure TXMLAttr.ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream);
type
...
begin
  ...
  //FNodeValue := UTF8Encode(ShrinkEol(Value));   //xxx
  FNodeValue := ShrinkEol(Value);   //xxx
end;

procedure TXMLCharacterData.SetText(const Value: XmlString);
begin
  //MUST NOT BE HERE: inherited;
  Data := Value;
end;

function TXMLCharacterData.GetNodeValue: XmlString;
begin
  //Result := UTF8Decode(FNodeValue);  //xxx
  Result := FNodeValue;  //xxx
end;
...
procedure TXMLCharacterData.InternalWriteToStream(const OutputStream: IUnicodeWriteStream);
begin
...
procedure TXMLCharacterData.ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream);
begin
  // do nothing
end;
...
procedure TXMLText.ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream);
type
  TParserState = (psText);
var
  ReadChar: XmlChar;
          ...
          '<':
            begin
              ...
              begin
                //ONDREJ
                xPreserve :=
                  (OwnerDocument.WhiteSpaceHandling in [wsPreserveAll, wsPreserveInTextOnly]) or//document default trim!!!
                  ((OwnerDocument.WhiteSpaceHandling = wsAutoTag) and (Parent is TXMLElement) and
                    (TXMLElement(Parent).PreserveWhiteSpace));//parent tag has set preserve attribute to true

                xNewValue := NodeValue + InputStream.GetOutputBuffer;
                if xPreserve then begin
                  if (OwnerDocument.WhiteSpaceHandling = wsPreserveInTextOnly) and StringIs_WhiteSpace(xNewValue) then
                    xNewValue := '';

                  if (Parent is TXMLElement) and (NodeValue <> '')
                    and (CharIs_WhiteSpace(NodeValue[1]) or CharIs_WhiteSpace(NodeValue[System.Length(NodeValue)]))
                  then
                    TXMLElement(Parent).PreserveWhiteSpace := True;
                end else begin
                  xNewValue := ShrinkWhitespace(xNewValue);
                  if (Parent is TXMLElement) then
                    TXMLElement(Parent).PreserveWhiteSpace := False;
                end;
                if xNewValue = '' then
...
begin
  ...
  //FNodeValue := UTF8Encode(Value);   //xxx
  FNodeValue := Value;   //xxx
end;

procedure TXMLCDATASection.InternalWriteToStream(const OutputStream: IUnicodeWriteStream);
begin
  // 2002-12-17 (mr): fixed indentation
...
procedure TXMLCDATASection.ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream);
type
...
procedure TXMLComment.InternalWriteToStream(const OutputStream: IUnicodeWriteStream);
begin
  // 2002-12-17 (mr): fixed indentation
  OutputStream.WriteIndent(True);
...
procedure TXMLComment.ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream);
type
  TParserState = (psData, psInEnd, psEnd);
var
  ReadChar: XmlChar;
...
procedure TXMLDocumentType.InternalWriteToStream(
  const OutputStream: IUnicodeWriteStream);
begin
  OutputStream.WriteIndent;
...
procedure TXMLDocumentType.ReadFromStream(const Parent: TXMLNode;
  const InputStream: IUnicodeReadStream);
type
...
procedure TXMLDocumentFragment.ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream);
begin
...
constructor TXMLDocument.Create(AParent: Pointer = nil);
begin
...
begin
  Result := '';
  raise EXMLException.Create('Don''t use IXMLDocument.Text property. Use IXMLDocument.XML instead.');
end;
...
var
  Stream: TVirtualMemoryStream;
begin
  Stream := TVirtualMemoryStream.Create;//MUST BE TVirtualMemoryStream -> SO THAT THE MEMORY POINTER WOULD NOT GET DESTROYED IN .Free!!!
  try
    {Stream.Write(PXmlChar(XML)^, Length(XML) * SizeOf(XmlChar));
    Stream.Seek(0, soFromBeginning);}
    Stream.SetPointer(PXmlChar(XML), Length(XML) * SizeOf(XmlChar));
    Result := _LoadFromStream(Stream, {$IFDEF FPC}CP_UTF8{$ELSE}CP_UTF16{$ENDIF});
  finally
    Stream.Free;
  end;
end;
...
{$IFNDEF OmniXML_NEXTGEN}
function TXMLDocument.LoadXML_UTF8(const XML: XmlRawByteString): Boolean;
var
  Stream: TVirtualMemoryStream;
begin
  Stream := TVirtualMemoryStream.Create;//MUST BE TVirtualMemoryStream -> SO THAT THE MEMORY POINTER WOULD NOT GET DESTROYED IN .Free!!!
  try
    //OLD AnsiString type
    Stream.SetPointer(@XML[1], Length(XML));
    Stream.Seek(0, soFromBeginning);
    Result := _LoadFromStream(Stream, CP_UTF8);
  finally
    Stream.Free;
  end;
end;
...
{$IFDEF OmniXML_D2009_UP}
function TXMLDocument.LoadXML_UTF8Bytes(const XML: TBytes): Boolean;
var
  Stream: TVirtualMemoryStream;
begin
  Stream := TVirtualMemoryStream.Create;//MUST BE TVirtualMemoryStream -> SO THAT THE MEMORY POINTER WOULD NOT GET DESTROYED IN .Free!!!
  try
    Stream.SetPointer(@XML[0], Length(XML));
    Stream.Seek(0, soFromBeginning);
    Result := _LoadFromStream(Stream, CP_UTF8);
  finally
    Stream.Free;
...
begin
  FURL := FileName;
  FS := TXmlFileStream.Create(FileName, fmOpenRead or fmShareDenyNone);
  try
...
function TXMLDocument.LoadFromFile(const FileName: string): Boolean;
begin
  Result := Load(FileName);
end;

function TXMLDocument.LoadFromStream(const Stream: TStream;
  const aForceEncoding: Word): Boolean;
begin
  Result := _LoadFromStream(Stream, aForceEncoding);
end;

function TXMLDocument.LoadFromStream(const Stream: TStream): Boolean;
begin
  Result := _LoadFromStream(Stream, 0);
end;

function TXMLDocument._LoadFromStream(const Stream: TStream; const aForceXMLEncoding: Word = 0): Boolean;
var
  xTS: TXMLReadStream;
...
begin
  ...
  FTextStreamForceEncoding := aForceXMLEncoding;
  xTS := TXMLReadStream.Create(Stream);
  try
    if aForceXMLEncoding > 0 then
      xTS.Encoding := GetCreateCodePage(aForceXMLEncoding);
    xTS.BlockFlushTempStream;
    ...
    try
      ...
      if FChildNodes = nil then//ONDREJ raise EXMLException.CreateParseError(0, MSG_E_MISSINGROOT, []) <- don't raise, just return FALSE
        Result := False;
    except
      on E: Exception do
      begin
...
procedure TXMLDocument.InternalWriteToStream(const OutputStream: IUnicodeWriteStream);
var
  i: Integer;
begin
  if HasChildNodes then
...
var
  ...
  begin
    ...
    if (PI <> nil) and FindCreateEncoding(PI, {%H-}CodePage) then
      Result := CodePage;
  end;
begin
  US := TXMLWriteStream.Create(OutputStream);
  try
    US.Encoding := InternalFindEncoding;
    US.OutputFormat := OutputFormat;
    InternalWriteToStream(US);
  finally
    US.Free;
...
var
  FileStream: TXmlFileStream;
begin
  FileStream := TXmlFileStream.Create(FileName, fmCreate);
  try
...
procedure TXMLDocument.ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeReadStream);
type
...
begin
      ...
      psTagName:  // one-time stop
        ...
        begin
          case ReadChar of
            ...
            // [40] STag
            // [5] Name
            //WE ALREADY PROCESSED PROCESSING INSTRUCTION WITH ENCODING -> UNBLOCK TEMP STREAM
            InputStream.UnblockFlushTempStream;
            if CharIs_Letter(ReadChar) or CharIs_Digit(ReadChar) or (ReadChar = '_') then//ONDREJ CharIs_Digit <- not XML standard, but sorry... for compatibility issues
            begin
              // it's an element
              InputStream.WriteOutputChar(ReadChar);
      ...
      psCommentOrCDATAOrDoctype:
        begin
          case ReadChar of
            '[':
              begin
                InputStream.GetNextString({%H-}Text, 6);
                if Text = 'CDATA[' then
                begin
                  InputStream.ClearOutputBuffer;
            ...
            '-':
              begin
                if InputStream.ReadChar(ReadChar) and (ReadChar = '-') then
                begin
  ...
  {if FChildNodes = nil then//ONDREJ NO PROBLEM, just tried to load an empty document...
...
procedure TXMLReadStream.BlockFlushTempStream;
begin
  fReader.BlockFlushTempString;
end;
...
function TXMLReadStream.GetEncoding: TEncoding;
begin
  Result := fReader.Encoding;
end;

function TXMLReadStream.GetNextString(var aReadString: XmlString;
  const aLength: Integer): Boolean;
var
  I: Integer;
  xReadChar: XmlChar;
begin
  SetLength(aReadString, aLength);
  i := 0;
  while (I < aLength) and ReadChar({%H-}xReadChar) do
  begin
    aReadString[I+1] := xReadChar;
    Inc(I);
  end;
  Result := (I = aLength);
end;
...
begin
  Result := fReader.ReadNextChar(aReadChar);
end;

procedure TXMLReadStream.SetEncoding(const aEncoding: TEncoding);
begin
  fReader.Encoding := aEncoding;
end;

procedure TXMLReadStream.UnblockFlushTempStream;
begin
  fReader.UnblockFlushTempString;
end;

procedure TXMLReadStream.UndoRead;
begin
  fReader.UndoRead;
end;
...
constructor TXMLWriteStream.Create(const aStream: TStream);
begin
  inherited Create;

  fWriter := TOTextWriter.Create(aStream);
  fWriter.WriteBOM := False;

  fIndent := -1;
end;

procedure TXMLWriteStream.DecreaseIndent;
begin
  Dec(FIndent);
  if FIndent = 0 then
    FIndent := MaxInt;
...
destructor TXMLWriteStream.Destroy;
begin
  fWriter.Free;
...
function TXMLWriteStream.GetEncoding: TEncoding;
begin
  Result := fWriter.Encoding;
end;

function TXMLWriteStream.GetOutputFormat: TOutputFormat;
begin
  Result := fOutputFormat;
end;

function TXMLWriteStream.GetWriteBOM: Boolean;
begin
  Result := fWriter.WriteBOM;
end;

procedure TXMLWriteStream.IncreaseIndent;
begin
...
procedure TXMLWriteStream.SetEncoding(const aEncoding: TEncoding);
begin
  fWriter.Encoding := aEncoding;
  fWriter.WriteBOM := (not aEncoding.IsSingleByte) and not (aEncoding is TUTF8Encoding);//DON'T WRITE BOM FOR UTF-8 XML!
end;

procedure TXMLWriteStream.SetOutputFormat(const Value: TOutputFormat);
begin
  fOutputFormat := Value;
end;

procedure TXMLWriteStream.SetWriteBOM(const Value: Boolean);
begin
  fWriter.WriteBOM := Value;
end;

procedure TXMLWriteStream.WriteIndent(const ForceNextLine: Boolean);
begin
...
procedure TXMLWriteStream.WriteString(const Value: XmlString);
begin
...
{ TXMLAttrListEnumerator }

constructor TXMLAttrListEnumerator.Create(aList: IXMLAttrList);
begin
  inherited Create;
  fIndex := -1;
  fList := aList;
...
function TXMLAttrListEnumerator.GetCurrent: IXMLAttr;
begin
  Result := IXMLAttr(fList.GetItem(fIndex));
end;

function TXMLAttrListEnumerator.MoveNext: Boolean;
begin
  Result := (fIndex < fList.Count - 1);
  if Result then
    Inc(fIndex);
...
procedure TVirtualMemoryStream.SetPointer(Ptr: Pointer; const Size: Longint);
begin
  inherited SetPointer(Ptr, Size);
end;

function TVirtualMemoryStream.{%H-}Write(const Buffer; Count: Longint): Longint;
begin
  raise Exception.Create('TVirtualMemoryStream.Write not implemented - should not be used!');
end;
